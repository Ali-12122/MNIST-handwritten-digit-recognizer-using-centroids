...
def preprocess_dataset(dataset: pd.DataFrame):
    ...
    for i in dataset.index:
        ...
        test_data_feature_vectors[i] = feature_vector
    ...
    preprocessed_dataset = pd.DataFrame(data=test_data_feature_vectors, columns=column_labels)
    preprocessed_dataset = pd.concat((preprocessed_dataset, dataset_labels), axis=1)

    return preprocessed_dataset


def preprocess_test_data(test_data: pd.DataFrame):
    # Facade function to execute the preprocessing
    # reshapes the each row such that it resembles a grayscale image
    # and applies extracts the feature vector from each.

    # Creating a matrix to store the feature vectors.
    test_data_feature_vectors = np.full((test_data.shape[0], 32), 0)

    # Applying the extract_feature_vector function on each
    # row after reshaping it to an image, then adding it to
    # the test_data_feature_vectors matrix
    for i in test_data.index:
        current_image = np.asarray(test_data.loc[i])
        current_image = np.reshape(current_image, (28, 28))
        feature_vector = extract_feature_vector(current_image, 16)
        test_data_feature_vectors[i] = feature_vector
    ...
    column_labels = []
    for i in range(0, 32, 2):
        x_c = str("Xc" + str(i + 1))  # Xc centroid
        y_c = str("Yc" + str(i + 1))  # Yc centroid
        column_labels.append(x_c)
        column_labels.append(y_c)

    # Creating the pandas.Dataframe containing the dataset
    preprocessed_test_data = pd.DataFrame(data=test_data_feature_vectors)
